    ...
    <c:forEach items="${data}" var="dt">
        <tr align="center">
            ...
            <td style="white-space: nowrap;">
                ...
                <c:if test="${dt.getStatus() != 0}">
                <select name="" id="" class="slctSts mdb-select" onchange="ubahSC('${dt.getId_subcomment()}_<c:if test="${dt.getStatus() == 1}">2</c:if><c:if test="${dt.getStatus() != 1}">1</c:if>')">
                    <option value="${dt.getId_subcomment()}_1" <c:if test="${dt.getStatus() == 1}">selected="true"</c:if>>Run</option>
                    <option value="${dt.getId_subcomment()}_2" <c:if test="${dt.getStatus() != 1}">selected="true"</c:if>>Block</option>
                    </select>
                </td>
                </c:if>
            </tr>
